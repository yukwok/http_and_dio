...
class HttpViewController extends GetxController {
  ...
  void httpRequest() async {
    ...
    try {
      ...
      var _json = convert.jsonDecode(resposnse.body) as List<dynamic>;

      print(_json);
      _json.forEach((element) {
        String temp = element['地區'] +
            element['大廈名單'] +
            element['個案最後到訪日期'] +
            element['相關個案編號'];
        uiListString.add(temp);

        print({temp});
      });
    } catch (e) {
      uiText.value = "Error Occurs:$e";
    }
  }
